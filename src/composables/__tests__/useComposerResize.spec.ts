...
describe("useComposerResize", () => {
  ...
  it("start → pointermove 更新高度（钳制 78..半窗，600 高下 max(128,96)-50），pointerup 清理", () => {
    ...
    expect(r.composerHeight.value).toBe(178);

    window.dispatchEvent(pointerEvent(-2000)); // 大幅上移 → 钳制半窗 300
    expect(r.composerHeight.value).toBe(300);

    window.dispatchEvent(pointerEvent(2000)); // 大幅下移 → 钳制最低 78
    expect(r.composerHeight.value).toBe(78);
