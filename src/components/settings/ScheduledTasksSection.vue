...
const runsState = reactive<
  Record<string, { items: TaskRunRecord[]; loading: boolean; done: boolean }>
>({});
const RUN_PAGE = 20;
...
const rows = computed<Row[]>(() => {
  ...
  if (done.length) {
    ...
    if (doneOpen.value)
      for (const task of done) out.push({ kind: "task", task });
  }
  return out;
});
...
async function toggleExpand(t: ScheduledTask) {
  ...
  if (!runsState[t.id])
    runsState[t.id] = { items: [], loading: false, done: false };
...
<template>
  <section v-show="active" class="settings-section settings-section-scheduled">
    ...
    <p class="settings-section-desc">
      到点后在绑定的原会话中自动发送任务指令；任务由 codex
      在对话中创建（动态工具 codexui.add_scheduled_task）。
    </p>
    <div class="model-config-card">
      ...
      <div class="sched-list">
        <div v-if="!store.scheduledTasks.length" class="plugin-empty">
          暂无定时任务。可在对话中让 codex 创建，例如：『每天早上 9
          点总结昨天的提交』。
        </div>

        <template
          v-for="row in rows"
          :key="row.kind === 'task' ? row.task.id : 'sched-done-head'"
        >
          <button
          ...
          >
            已完成（{{ store.scheduledTasks.filter((t) => t.done).length }}）{{
              doneOpen ? "▲" : "▼"
            }}
          </button>
          <div
            v-else
            class="sched-task"
            :class="{ expanded: expandedId === row.task.id }"
          >
            ...
            <div v-if="expandedId === row.task.id" class="sched-detail">
              ...
              <div
                v-if="runsState[row.task.id]?.items.length"
                class="sched-runs"
              >
                <div
                  v-for="r in runsState[row.task.id].items"
                  :key="r.id"
                  class="sched-run"
                >
                  <div class="sched-run-line">
                    ...
                    <span
                      v-if="r.durationMs != null"
                      class="sched-run-duration"
                    >
                      {{ fmtDuration(r.durationMs) }}
                    </span>
                    <span
                      v-if="r.error"
                      class="sched-run-error"
                      v-tooltip="r.error"
                      >{{ r.error }}</span
                    >
                  </div>
                  ...
                  >
                    {{ resultOpen.has(r.id) ? r.result : oneLine(r.result) }}
                  </div>
                </div>
                <button
              ...
              <div
                v-else-if="!runsState[row.task.id]?.loading"
                class="sched-run-more"
              >
                暂无执行记录
              </div>
    ...
    >
      <div class="sched-edit-form">
        ...
        <div class="setting-row sched-edit-readonly">
          <label> 调度表达式 </label>
          <span class="sched-edit-readonly-value">{{
            describeSchedule(editTask.cron)
          }}</span>
        </div>
        <div class="setting-row sched-edit-readonly">
          <label> 绑定会话 </label>
          <span class="sched-edit-readonly-value">{{
            threadLabel(editTask.threadId)
          }}</span>
        </div>
      </div>
      <template #foot>
        <button class="btn" :disabled="editSaving" @click="closeEdit">
          取消
        </button>
