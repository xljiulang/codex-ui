...
describe("TASK_MODES 任务模式清单", () => {
  it("仅包含执行/计划两种模式（目标已从任务模式移除）", () => {
    expect(TASK_MODES.map((m) => m.id)).toEqual(["default", "plan"]);
  });
  ...
  it("taskMode() 对未知 id 回退到执行模式（default）", () => {
    expect(taskMode("goal").id).toBe("default");
    expect(taskMode("default").id).toBe("default");
