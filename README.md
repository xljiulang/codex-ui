...
- **滚动体验**：流式输出时上滑查看历史不会被拉回底部，右上角“回到底部”按钮一键恢复跟随；历史会话打开后先在后台完成全文预热，随后滚动条左侧出现导航按钮（固定显示四宫格图标，Windows 开始菜单风格；回合进行中图标旋转提示——等待响应很慢速、执行工具中常速，鼠标悬停时原地暂停）——悬停按钮即展开回合导航卡片、移出自动收起（键盘 Enter/Space 也可开合），卡片按时间正序以单行紧凑列出每个回合（纯数字序号 + 标题单行省略（不设字数截断）+ 时间，悬停条目直接在卡片旁预览该条用户消息气泡，当前回合高亮；用户消息为“执行计划”指令时标题直接取该计划标题），卡片底部为底栏：token 输入/输出芯片（靠左，代币图标 + ↑/↓ 箭头数值（悬停显示精确值））与一枚合并胶囊（靠右，窗口图标 + 上下文 `X / Y` 用量（悬停显示已用/最大） + 压缩按钮左缘分隔 + 向中心压缩的四向图标，一律全圆角），点条目即跳转到该回合起点（目标消息对齐视口顶部并短暂高亮）；聊天滚动区底边（输入区上方）常驻一条横贯全宽的上下文占用进度条（主题色 2px 细条 + 发光，填充宽度=已用/窗口百分比，数值由导航底栏胶囊承载）。
