...
- **会话资源**：右侧面板底部双 Tab（历史会话 / 会话资源）；以**当前会话工作目录**为根的文件树，**懒加载、默认只展开第一层**，名称以 `.` 开头的目录自动隐藏；头部搜索框递归搜索文件/目录名；**文件变化自动刷新**（`notify` 监听、300ms 防抖）；右键菜单管理（复制/粘贴、重命名、删除、属性、在资源管理器中打开、添加为会话附件），支持从系统剪贴板粘贴文件/文件夹，属性弹窗显示大小与时间；切换 Tab 保留展开状态。
