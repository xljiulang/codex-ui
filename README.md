...
- **插件管理**（设置页「插件管理」Tab）：顶部「已安装插件」卡片跨市场汇总已安装插件（图标、版本、状态、来源市场，可直接卸载），无已安装插件时显示空态；下方插件市场按市场分组列出插件，每个插件展示接口图标（远程 URL 优先、本地路径其次，缺失或加载失败时以品牌色 + 首字母占位）；支持安装/卸载、添加/移除市场。
