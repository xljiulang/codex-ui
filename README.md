...
附带 CLI 工具提示：codex-ui 会探测 codex 进程 PATH 上可用的 `rg`/`fd`/`ast-grep`，并在默认（执行）协作模式的回合内向 agent 注入这些工具的使用说明（文本搜索、快速查找、AST 搜索改写）；计划模式仍用 codex 自身的计划指令，不受影响。
...
- **滚动体验**：流式输出时上滑查看历史不会被拉回底部，右上角“回到底部”按钮一键恢复跟随；历史会话打开后先在后台完成全文预热，随后滚动条左侧出现导航按钮——悬停按钮即展开回合导航卡片、移出自动收起（键盘 Enter/Space 也可开合），按时间正序以单行紧凑列出每个回合（纯数字序号 + 标题单行省略 + 时间，标题溢出时悬停显示 200 字内截取文本 tooltip（在卡片左侧），当前回合高亮；用户消息为“执行计划”指令时标题直接取该计划标题），点条目即跳转到该回合起点（目标消息对齐视口顶部并短暂高亮）。
...
- **欢迎介绍标签**：**启动时按「个性化 → 启动时打开介绍标签页」设置打开**（默认开启）「欢迎」标签（纯静态图文导览，无跳转交互），分区介绍核心功能——会话管理与微信远控、对话内 codexui 动态工具（微信远控下自查 token 用量并主动压缩上下文）、资源管理（可打开/预览的文本与代码、Markdown、.docx、.pdf、常见图片、.xlsx 及 Git diff）、Git、`logs_2.sqlite` 日志写阻断防膨胀、内置 `openai-bundled` / `openai-primary-runtime` 双插件市场、多会话并行标签、终端——并附更多亮点（历史分组/分叉、自动标题、权限/任务模式、三套主题、技能/MCP/插件、本地记忆、rg/fd/ast-grep 工具提示等）；标签可手动关闭（不持久化，重启按设置再次打开），全部标签关闭后的空状态提供「查看 Codex-UI 介绍」按钮随时重开。欢迎页在默认 1280 窗口下占满内容区、卡片更大。
...
- **模型与上下文**：模型下拉选择（`model/list`）、推理强度跟随模型动态生成（如 `low/high/max`）、模型按钮显示“名称 (强度)”（权限/任务/模型按钮弹出层点击外部自动关闭、三个弹出层互斥）、修改模型/推理强度应用后立即通过 `thread/settings/update` 同步到当前会话（无会话时由新建会话的 `thread/start` 携带）、上下文窗口百分比椭圆框（悬停显示“上下文已用 xx K，共 yy K”，点击二次确认后手动压缩上下文 `thread/compact/start`）；对话面板右上角固定显示本会话累计输入/输出 token 数（`thread/tokenUsage/updated` 的 `total` 字段，上/下箭头区分输入/输出）。
