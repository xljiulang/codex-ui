...
设置页新增「微信接入」分区：经**微信官方 ClawBot 通道**（ilink bot API）把你的 Codex 带进微信聊天——手机扫码授权后，直接在 ClawBot 联系人里发消息，就会驱动本机 codex-ui 中的 Codex 完成回合，并把回复以你绑定的账号被动发回，实现「人在外面、Codex 在家里干活」的远控体验。

启用步骤：
1. 在本机安装 Node.js（与 codex 的 npm 安装前提一致）；
2. 打开设置 →「微信接入」，打开「启用」开关；
3. 点击「扫码登录」并扫描二维码 —— 完成。门禁为**谁扫谁白**：只有扫码绑定的这个微信号的消息会交给 Codex，其他联系人的消息一律忽略并记录，无需填写任何 ID。
...
- **免审批完整能力 + 本人门禁**：微信触发的线程固定使用「完全访问 + 免审批」（cwd 固定为应用启动工作区），关闭总开关立即失效；由于门禁只放行扫码绑定账号本人，请勿将该微信出借他人使用。
- **在线范围**：bot 仅在 codex-ui 运行期间在线；窗口关闭即停止长轮询并使 bot 离线。会话意外退出时会静默重启一次并复登已有账号。
- **凭据存储**：登录 token 由 sidecar 保存在 `%APPDATA%\com.codexui.app\wechat\wechannel-data\`（应用数据目录内）；`peer ↔ 线程` 映射存于同目录 `threads.json`。
- **排队规则**：同一联系人的消息串行执行（FIFO，上限 16 条），溢出时回一条忙碌提示；单回合最长等待 10 分钟，超时按失败回复。

排查提示：若绑定后大号/小号聊天无响应而状态已显示「已连接」，查看应用日志中「忽略非本人消息：<ID>」——如该 ID 就是你的使用账号（个别版本平台可能不回传统一 userId），属协议行为差异；当前版本暂未提供放行开关，可在 GitHub 反馈补充场景信息。
