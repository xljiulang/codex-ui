...
- **出站发文件（微信回合专属，⚠️ 当前不可用）**：在微信里让 Codex 把本地文件发回来（例如「把 `D:\x\报告.pdf` 发给我」）时，微信回合会向模型注入一条通道说明，要求它在回复正文之后**单独写一行** `<wechat_send_file>文件的绝对路径</wechat_send_file>`（多个文件写多行）；该标签不展示给用户，应用在回合收尾按标签把文件加密上传到微信 CDN 并发一条媒体消息（图片/视频按扩展名走对应类型，其余按文件），标签本身从正文删除、正文照常先发。单条回复最多 5 个文件、单个文件明文上限 200 MB；路径不存在/是目录/超限时回一条 `⚠️ 无法发送文件：…`；标签写在 ``` 代码块内不会触发发送。**仅微信回合注入该说明**：桌面端与定时任务的回合不注入、也不响应标签。
  **但接收端目前打不开这些附件**（实测：消息能在微信里显示，点击却提示「图片已过期或者已清理」，文件卡在「下载中 0%」）。已定位到的原因是平台侧的下载参数自相矛盾——上传响应头 `x-encrypted-param` 填进消息**才会投递/显示**，但它拿去 `/download` 恒返回 `400 invalid encrypted_param`；而 `getuploadurl` 返回的 `upload_param` 能正常取回对象（本地实测可下载并解密还原），填进消息却导致消息**完全不显示**。两个候选参数各缺一半，且平台未提供公开协议文档、唯一的 MIT 参考实现在这一点上同样是错的。后续若要修，需要拿到 `x-encrypted-param` 的正确解引用方式（或确认消息里应当填第三种值）。
...
- **在线范围**：bot 仅在 codex-ui 运行期间在线；窗口关闭即停止长轮询并使 bot 离线。接收器因可恢复原因（如 getUpdates 连续失败）意外退出时，只要账号仍绑定且未过期，会在 5 秒后**自动重启一次**并恢复接收；会话过期（需重新扫码）与主动解绑不自动重启。自动重启每个账号限一次，下次绑定/解绑后重新计。
