...
- **欢迎介绍标签**：**启动时按「个性化 → 启动时打开介绍标签页」设置打开**（默认开启）「Codex-UI 介绍」标签（纯静态图文导览，无跳转交互），分区介绍核心功能——会话管理与微信远控、对话内 codexui 动态工具（微信远控下自查 token 用量并主动压缩上下文）、资源管理（可打开/预览的文本与代码、Markdown、.docx、.pdf、常见图片、.xlsx 及 Git diff）、Git、`logs_2.sqlite` 日志写阻断防膨胀、内置 `openai-bundled` / `openai-primary-runtime` 双插件市场、多会话并行标签、终端——并附更多亮点（历史分组/分叉、自动标题、权限/任务模式、三套主题、技能/MCP/插件、本地记忆、系统托盘常驻、rg/fd/ast-grep 工具提示等）；标签可手动关闭（不持久化，重启按设置再次打开），全部标签关闭后的空状态提供「查看 Codex-UI 介绍」按钮随时重开。
