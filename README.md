...
- 新建会话通过 `thread/start.dynamicTools` 注册 `codexui` 命名空间工具，agent 可在对话内直接调用：
  ...
  - `codexui_compact_context`（压缩上下文）与 `codexui_add_scheduled_task`（创建定时任务，见下）。
...
- **AI 看门狗只在默认模式跑**：用**同一个模型**发一次独立的后台判定调用（非流式、不带任何工具，提示词含本轮用户请求、助手终局文本与「本回合工具调用数为 0」的事实，要求只回一行 `未完成`/`已完成`；
  判定规则——**方案/计划本身就是交付物、明确等你确认也算已完成**，
  只有「声称现在去做却既没有工具调用也没有交付物」才判未完成；
  提示词里**不再出现 `<proposed_plan>` 字样**），判为「未完成」就把提醒作为**合成 user 消息**追加到发给上游的历史里
