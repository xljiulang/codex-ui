...
- `+` 菜单、`@`/`$` 菜单；
- **上下键选择输入历史**（内存）；
- **Ctrl+V 粘贴 / 文件拖放到输入区添加附件**（截图/图片落盘到临时目录，复制文件用原始路径）；**粘贴网页链接时自动转成「标题 + URL」纯文本**（避免 ProseMirror 只保留链接标题、URL 丢失）；
...
- 其后可打开文件（CodeMirror 编辑）、diff、PDF/图像预览与 cmd / PowerShell 终端标签；
...
- 关闭仍在运行的会话标签或终端需确认；关闭主窗口则直接隐藏到系统托盘，不会触发此确认。

### 终端标签

- 资源管理器文件夹与历史会话分组的「在此打开终端」按「设置 → 基础设置 → 终端 Shell」启动终端（默认 cmd，可选 PowerShell；PowerShell 优先使用 pwsh 7+，未安装时回退 Windows PowerShell 5.1）。Windows ConPTY、UTF-8 编码，支持同目录多开、切走不中断；标签标题随 Shell 显示为「终端 (cmd)」或「终端 (PowerShell)」（无 ToolTip）；
- 编辑区标签栏末尾「+」菜单可单次选择新建 cmd 或 PowerShell 终端，不受默认 Shell 设置影响；
...
- 切换会话标签不中断后台回合；关闭仍在运行的标签需确认，托盘退出则按应用退出流程静默停止工作会话和运行中的终端。
...
- 设置以独立标签显示，左侧为分类导航。个性化偏好与基础设置项均即时保存；模型配置、兼容代理等需要显式保存的页面会提供对应按钮。

### 主题与外观

- **三套主题（蓝夜/曜黑/晨光）点击卡片即时预览并自动保存**，主题统一为 Raycast/Linear 式光影质感（悬浮阴影、毛玻璃、主色辉光与背景氛围光，浅色主题保持通透明亮）。
...
- 关闭开关即恢复纯色主题外观。
- Enter 快捷发送与会话错误、提权/交互/完成通知开关也位于「个性化」；偏好项变更后即时保存，系统通知只在主窗口没有前台焦点时发送。

### 基础设置（通用设置）

- 基础设置中的 codex 可执行文件路径、默认终端 Shell、默认权限与跟进处理方式（调整方向 / 加入队列）均在更改后自动保存；Shell 设置只影响之后新建的终端，默认权限用于新会话。
- 终端 Shell 默认是 cmd，也可选 PowerShell；PowerShell 优先使用 pwsh 7+，未安装时回退 Windows PowerShell 5.1。
...
- 新建会话通过 `thread/start.dynamicTools` 注册 `codexui` 命名空间工具，agent 可在对话内直接调用：
  ...
  - `codexui_send_file_to_wechat`（把本机文件发送到已绑定的微信账号，见「微信接入」）。
- 工具调用经 `item/tool/call` 回由 codex-ui 应答：前三条在前端处理，`send_file_to_wechat` 由**后端 Rust 直接应答执行**（微信回合可能无人值守，不依赖窗口存活）。设置页「动态工具」区块可逐个启停（实验性、依赖 `experimentalApi`，仅新建会话注入）。
...
│   ├── terminal.rs    ConPTY 终端会话（portable-pty 启动 cmd 或 PowerShell，输出事件流/输入/缩放/kill，支持多开）
│   ├── diff.rs        diff 预览行构建
...
- **多会话标签**：标签栏末尾「+」菜单可新建空会话（使用活动标签的工作区，无则空）、打开文件（系统文件选择器多选，文本/预览类型应用内打开，不支持的类型提示无法打开；初始定位为当前工作区），或直接新建 cmd / PowerShell 终端（无确定工作区时回退到应用自身目录）；历史目录行的「在此打开终端」使用设置里的默认 Shell。头部「+」先选目录再新建会话。会话标签可关闭，允许关闭到 0 个（主区域显示空状态提示）。多开会话时**文件拖放仅当前活动会话的输入框接收为附件**（隐藏标签输入区不响应），拖到其它任意位置仍按「打开文件」处理。
- **会话列表**：面板顶部即搜索框；按目录分组（目录按名称 A-Z、会话按置顶+时间倒序），文件夹默认收起，支持全量加载、重命名、**分叉会话**（把源会话完整复制为新线程、打开新标签并沿用源名称，可在此基础上另起分支继续）、固定置顶、删除确认。
  点击会话行：未打开则新标签打开、已打开则聚焦对应标签（点击当前会话不重载）；已打开会话行显示「已打开」标记、后台运行中显示呼吸点；切换标签不中断后台回合；关闭仍在运行的会话标签或终端需确认，关闭主窗口则直接隐藏到系统托盘，不会触发此确认。
...
- **设置文件**：`%APPDATA%\com.codexui.app\settings.json`，仅保存 codex 路径、Enter 快捷发送、跟进处理方式、
  两个系统通知开关（错误 `error_notify_enabled` / 提权·交互·完成 `interaction_notify_enabled`）、主题、毛玻璃特效开关、权限模式初始值、记忆模式与最后活跃会话 id（`last_session_id`，供下次启动恢复）。微信按会话绑定，不使用全局开关。
...
2. 打开弹窗后会自动生成二维码，使用要绑定的微信扫码并确认授权 —— 完成。门禁为**谁扫谁白**：只有扫码绑定的这个微信号本人发来的消息会交给该会话，其他联系人的消息一律忽略并记录，无需填写任何 ID。
...
- **文本、图片、文件与视频**：文本消息直接驱动回合；
  ...
  **引用消息中的附件同样接收**：引用一条含图片/文件/视频的消息转发时，被引用消息里的附件按同样规则下载并交给 Codex（只解一层引用，更深层不展开）；被引用消息的正文不当作本轮指令。
...
- **「对方正在输入」指示**：收到消息进入回合即向 ClawBot 返回正在输入状态（`sendtyping status=1`），回合期间每 8 秒续发保持可见，回复送达/结束后取消（`status=2`）；codex app-server 断开重连瞬间即停止续发（不再闪烁），全程 best-effort，失败仅记日志不影响回复。
- **免审批完整能力 + 本人门禁**：微信触发的回合固定使用「完全访问 + 免审批」（文件与网络不受限）；由于门禁只放行扫码绑定账号本人，请勿将该微信出借他人使用。
- **在线范围**：bot 仅在 codex-ui 运行期间在线；关闭主窗口只会隐藏到系统托盘，微信仍保持在线，直到从托盘退出应用。接收器因可恢复原因（如 getUpdates 连续失败）意外退出时，只要账号仍绑定且未过期，会在 5 秒后**自动重启一次**并恢复接收；会话过期（需重新扫码）与主动解绑不自动重启。自动重启每个账号限一次，下次绑定/解绑后重新计。
- **凭据存储**：登录 token 由应用保存在 `%APPDATA%\com.codexui.app\wechat\wechannel-data\`（应用数据目录内）；`会话 ↔ 微信账号` 绑定存于应用数据目录的 `sessions.json`（与权限模式/模型等会话状态同一文件，键为会话 id）。旧版的 `wechat\bindings.json` 仅作为迁移来源，首次加载时自动并入后删除，新版不再写入。
  该目录下的 `media\<会话id>\` 存放微信发来的图片 / 文件 / 视频；旧版按 `media\<年-月>\` 落盘的文件保留原位，程序不再写入也不再读取。
- **无队列，回合中消息自动并入**：微信通道不排队，也不会因等待过久而判失败——**单回合不设等待上限**，只有 codex app-server 断开才会停止「正在输入」续发（回合仍继续等待，不发失败消息）。
  同一联系人在回合进行中再发消息时，用协议 `turn/steer` **并入当前回合**（与设置页「跟进处理方式 = 调整方向」同义）：**这类消息不再各自产生一次回复**，而是随该回合的最终答复一起返回；服务端拒绝转向（`/review`、手动压缩等回合）或转向失败时回一条失败提示。
  同一会话的消息按到达顺序串行处理；**不同绑定会话之间互相并行**，一个长回合不会挡住其它微信账号。
