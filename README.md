...
- **聊天流**：流式 Markdown（表格、任务列表、代码块复制按钮、链接悬停显示完整 URL、图片）、思考过程（流式展开、完成折叠、耗时显示）、进行中阶段徽标与记忆引用、计划/待办、上下文压缩提示。
- **工具卡片**：命令执行显示真实命令、工作目录、实时输出、**实时耗时**、退出码；文件变更带 diff；网络搜索（结构化结果列表）、MCP/动态工具、子代理协作。
- **交互弹窗**：提权审批（批准 / 拒绝 / 本次会话批准 / 批准并记住此规则）、选项提问、MCP 表单；弹窗出现时播放提示音并置前窗口；回合可随时停止并提示“已停止生成”。
- **历史会话**：展示全部目录的会话并标注目录；打开为只读，发消息才恢复；继续执行使用会话记录的工作目录。
- **权限模式**：请求批准 / 帮我批准 / 完全访问权限（对话进行中按钮禁用）。
- **任务模式**：执行 / 计划 / 目标（目标对话框）。
- **输入区**：通栏输入框 + 悬浮靠底按钮；`+` 菜单、`@`/`$` 菜单；**上下键选择输入历史**（内存）；本地图片附件在消息中直接渲染（输入区带缩略图）。
- **设置**：codex 路径、Enter 快捷发送、跟进处理方式（调整方向 / 加入队列）、提示音开关（权限模式、模型、推理强度在输入区按钮菜单中配置并持久化）。
...
- **工作目录**：从哪个目录启动 codex-ui，新建会话就归到哪个目录；历史会话显示各自记录的目录。
- **历史会话**：点击仅查看；输入消息后才会恢复并执行（执行目录为会话记录的 cwd）。带**活跃目标**的会话恢复后会按目标模式自动持续执行——不需要时可点“目标”chip 清除目标。
- **权限模式**：请求批准会询问所有外部写操作；“批准并记住此规则”会把规则写入 codex 策略。完全访问不受限。
- **设置文件**：`%APPDATA%\com.codexui.app\settings.json`（codex 路径、Enter 快捷发送、提示音；权限模式、模型、推理强度由输入区菜单设置后同样持久化到该文件）。
