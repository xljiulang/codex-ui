...
- **滚动体验**：流式输出时上滑查看历史不会被拉回底部，右上角“回到底部”按钮一键恢复跟随；历史会话打开后先在后台完成全文预热，随后滚动条左侧出现导航按钮（固定显示四宫格图标，Windows 开始菜单风格；回合进行中图标旋转提示——等待响应很慢速、执行工具中常速，两种状态换挡沿用当前角度继续旋转，鼠标悬停时图标归位静止、移出后重新起转）——悬停按钮即展开回合导航卡片、移出自动收起（键盘 Enter/Space 也可开合），卡片按时间正序以单行紧凑列出每个回合（纯数字序号 + 标题单行省略（不设字数截断）+ 时间，悬停条目直接在卡片旁预览该条用户消息气泡，当前回合高亮；用户消息为“执行计划”指令时标题直接取该计划标题），点条目即跳转到该回合起点（目标消息对齐视口顶部并短暂高亮）。
