...
- **滚动体验**：流式输出时上滑查看历史不会被拉回底部，右上角“回到底部”按钮一键恢复跟随；历史会话打开后先在后台完成全文预热，随后滚动条左侧出现导航按钮——点击展开回合导航卡片，按时间正序以单行列出每个回合（序号圆点 + 标题单行省略 + 时间，标题溢出时悬停全文 tooltip 显示在卡片左侧，当前回合高亮），点条目即跳转到该回合起点（目标消息对齐视口顶部并短暂高亮）。
