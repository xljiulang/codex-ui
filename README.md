...
- **AI 看门狗只在默认模式跑**：用**同一个模型**发一次独立的后台判定调用（流式、不带任何工具，提示词含本轮用户请求、助手终局文本与「这条回复里没有调用任何工具」的事实，要求只回一行 `未完成`/`已完成`；
  判定规则——**验收口径按任务类型中立**：用户请求可以是任何类型（写代码、改文件、**执行命令、git 提交与推送**、查询或解释、发布、删除等），只要助手给出了明确结论、**结果说明**或交付物（如「已提交并推送，commit 是 abc123」）就算已完成；
  **方案/计划本身就是交付物、明确等你确认也算已完成**；只有「只是承诺接下来/马上去做（将来时），既没给出结果说明或交付物、也没说要等你确认」才判未完成；
  「本轮这条回复没有工具调用」被显式限定为**只描述这一条回复、不能推断整个任务没做**——操作型指令可能在更早的回合就已经用工具执行过，本轮只是如实汇报（此前缺这层说明会把「请 git 提交和推送代码」判成未完成并触发续跑）；
  提示词里**不出现 `<proposed_plan>` 字样**），判为「未完成」就把提醒作为**合成 user 消息**追加到发给上游的历史里
...
- 边界与开关：判定拿不准（超时 60 秒／报错／输出无法解析）一律按「已完成」处理，照常收尾；同一会话「连续注入且模型仍未实际调用工具」达到 **2 次**后不再注入（模型一旦真的调用工具、或距上次注入超过 10 分钟即清零），单次入站请求最多 4 个 pass 兜底；请求里没有声明任何工具时不做判定；**非判定路径、错误/失败路径、历史净化与工具翻译逻辑均不受影响**。
- 诊断日志：`zen_proxy.request` 的 `mode=plan|default` 与 `mode_src=registry|heuristic`（本次识别出的协作模式及其来源，排查「计划模式仍发起判定」或「默认模式被当成计划模式」先看这两格）、
  ...
  `zen_proxy.nudge_skipped`（原因：plan_output／plan_cancelled／plan_unachievable／title_task／上游错误/状态码/转发失败）、
