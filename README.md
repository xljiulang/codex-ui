...
- **权限模式**：只读访问（文件只读、可联网检索，不会修改任何文件）；请求批准会询问所有外部写操作；“批准并记住此规则”会把规则写入 codex 策略。完全访问不受限。权限/模型/推理强度为**会话级**，每个会话标签独立配置，新会话取默认（模型未显式选择时由模型列表默认值决定）。
...
- **免审批完整能力 + 本人门禁**：微信触发的回合固定使用「仅限工作目录的完全访问 + 免审批 + 允许联网」（会话工作目录内可读写、目录外只读；读不到工作目录时降级为只读）；由于门禁只放行扫码绑定账号本人，请勿将该微信出借他人使用。
