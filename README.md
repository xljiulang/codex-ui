...
- 右侧依次为「设置（中性灰 16px 图标按钮）」「模型快照（纯图标单色外框 + `M`，下拉：列出 `CODEX_HOME/codex-ui` 下的模型快照，还原 / 删除（确认）/ 打开 / 底部新建并覆盖）」「切换布局（左右布局 ⇄ 隐藏面板两态图标，点击在显示/隐藏右侧面板间切换，隐藏时编辑区占满全宽且面板内容状态保留，仅本次运行生效不持久化）」「最小化 / 最大化(还原) / 关闭」（关闭悬停变红）；
