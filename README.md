...
- **会话资源**：右侧面板底部 Tab（历史会话 / 会话资源）；以**当前会话工作目录**为根的文件树，**懒加载、默认只展开第一层**，名称以 `.` 开头的目录自动隐藏；头部搜索框递归搜索文件/目录名；**文件变化自动刷新**（`notify` 监听、300ms 防抖）；右键菜单管理（复制/粘贴、重命名、删除、属性、在资源管理器中打开、添加为会话附件），支持从系统剪贴板粘贴文件/文件夹，属性弹窗显示大小与时间；切换 Tab 保留展开状态。
- **Git 更改**：右侧面板底部 Tab（历史会话 / 会话资源 / **Git 更改**），依赖系统 `git`；以**当前会话工作目录**为根检测所在仓库，列出**已更改文件**（新增/修改/删除/重命名/未跟踪/冲突徽标 + 分支名），点击文件复用独立 diff 窗口查看（未跟踪文件以空内容对比、其余统一对比 HEAD）；**变更自动刷新**（监听仓库目录与 `.git`，300ms 防抖）；目录不在仓库时提供**“添加到 Git”一键初始化**（仅 `git init`，不自动提交），未安装 Git 时显示对应空状态。
...
│   ├── git.rs         Git 更改：状态/diff/初始化 + .git 监听（调用系统 git CLI）
│   └── settings.rs    设置读写
└── Vue 3 前端  src/
...
- `git`（“Git 更改”面板依赖，需支持 `--porcelain=v2`，建议 ≥ 2.20）
