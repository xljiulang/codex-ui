...
- **会话自动提示**：codex-ui 在每个回合的 `turn/start` 里通过 `additionalContext`（来源标识 `codexui-kb`）附带一段固定的用法说明，因此 agent 在每个会话（含定时任务、微信桥发起的回合）都知道本机有 `codexui-kb` 可查；说明只讲命令用法与使用时机，**不绑定工作目录**——选哪个库由 agent 依用户指名、库名语义或向你确认来决定。
