...
- 关闭开关即恢复纯色主题外观；
- codex 路径（留空自动查找，设置页显示当前检测到的路径、仅展示不写入配置）、模型配置内置 DeepSeek / GLM 接入文档链接（**模型目录 `model_catalog_json` 路径自动管理**——无需手填，按 config 实际值解析、未配置时默认 `CODEX_HOME/models.json` 绝对路径，保存时校验目录 JSON 结构、目录为空则自动删除该配置键、模型目录编辑框不可拖拽缩放；模型目录标签在文件存在时显示「在编辑器中打开文件」文字链接、点击在编辑器中打开该 json 文件；**模型名称输入框点击展开全量候选**，由前端解析模型目录 `slug`（缺失回退 `id`）得到、**不做按已填值过滤**，点选即回填；模型提供方行在同时填写 `base_url` 与 `experimental_bearer_token` 时显示圆圈+右箭头图标（Material arrow_circle_right）的「生成模型目录」按钮，点击后用该提供方 `/models` 获取全部模型并打开选择弹窗：默认全不选，候选分为可生成/不兼容/未匹配，后两类置灰但保留具体原因；命中官方条目池时整条复用，第三方中转只把 `prefer_websockets` 置 false——条目池 = **启动时从本机 codex 导出的自带目录**（`codex debug models --bundled` 写入 `%APPDATA%\com.codexui.app\cache\codex-models.json`，GPT 条目因此始终与用户安装的 codex 版本匹配，不再内置研发环境导出的 GPT 条目）+ `official-models.json` 里手写的第三方官方条目（同 slug 时导出条目优先）；**生成条目的模板也跟随本机 codex**（渲染模板 = 该导出目录第一条作基底 + `model_catalog_template.json` 覆盖，提示词随版本；条目缺必需字段会让 codex 整份目录解析失败，故写出前还会对每条输出「只补不覆盖」地补齐模板键）；models.dev 只按模型 ID 匹配（`catalog.json` 的全量 provider 展开、不按 `base_url` 定位分组、不体现提供方，本机代理也能命中 Zen 的 `big-pickle` 这类专有 ID），OpenRouter 最后补缺，低质量来源不会覆盖高质量字段；同系列未知新版本可继承最新旧版本资料（如 GPT-5.x、Qwen Coder、DeepSeek V 系列），并显示「继承自」来源；支持搜索与三态「全选」复选框，确认后仅写入选中模型。字段生成采用保守能力默认值，识别输入上限、工具调用与 beta/deprecated 状态，保留自定义推理档位；`display_name` 仍只按提供方原始 slug 生成，不读取数据源名称，也不自动保存。详见 [docs/model_catalog_json.md](docs/model_catalog_json.md)；模型配置新增 **`personality`（回复风格：friendly 亲和 / pragmatic 务实简洁 / none，选 pragmatic 可减少输出 token）** 与 **`model_verbosity`（输出详细程度：low / medium / high，控制回答长短、不影响推理，选 low 可减少输出 token）** 两个下拉，均支持「默认（不写入）」（空值写 null 删除该键回退内置默认），随模型配置一并经 `config/batchWrite` 热重载保存）；
