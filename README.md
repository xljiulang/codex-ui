...
- **文本与图片**：文本消息直接驱动回合；图片消息按微信 CDN 协议下载并 AES-128-ECB 解密后，落到应用数据目录 `%APPDATA%\com.codexui.app\wechat\wechannel-data\media\<年-月>\`（永久保留、不自动清理，需要时自行清理该目录），再作为本地图片附件随该回合交给 Codex 识别——图片+文字同一条消息时一起发送，纯图片消息则不带任何文本。单条消息最多 4 张、单张不超过 20 MB，超出或接收失败会在微信里回一条提示。语音 / 文件 / 视频消息仍自动忽略；回复为最终 agent 文本，超长自动分段（约 1800 字/条）。
