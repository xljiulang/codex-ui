...
- **Git 更改**：右侧面板底部 Tab（历史会话 / 会话资源 / **Git 更改**），后端内置 **gix**（纯 Rust，不依赖系统 Git）；以**当前会话工作目录**为根检测所在仓库，列出**已更改文件**（新增/修改/删除/未跟踪圆形状态图标，悬停提示状态文字 + 分支名），点击文件复用独立 diff 窗口查看（统一对比 HEAD，暂存 + 未暂存合并；未跟踪文件以空内容对比）；**变更自动刷新**（监听仓库目录与 `.git`，300ms 防抖，`.gitignore` 规则参与事件过滤）；目录不在仓库时提供**“添加到 Git”一键初始化**（仅 `git init`，不自动提交）。
