...
- **历史会话**：面板**常驻右侧且宽度可拖拽调节（264px ~ 窗口一半）**，展示全部目录的会话并**按目录分组为文件夹（目录按名称 A-Z，会话按置顶+最近时间倒序；默认收起，单条会话不建夹、不显示路径）**；**全量加载**、重命名、固定置顶（行首显示钉子图标）、搜索（带摘要，搜索框右侧刷新按钮）、删除确认，会话行**右键菜单（加载/重命名/置顶固定、取消固定/删除会话）**，目录（文件夹）行右键可**创建会话（预置该分组目录）**或**在资源管理器中打开**；创建会话后自动聚焦输入框；打开为只读，发消息才恢复；点击会话不自动关闭、点击当前会话不重载；继续执行使用会话记录的工作目录，历史中的命令/工具详情完整加载。
- **会话资源**：右侧面板底部 Tab（历史会话 / 会话资源）；以**当前会话工作目录**为根的文件树，**懒加载、默认只展开第一层**，名称以 `.` 开头的目录与 `node_modules` 无条件隐藏，**根节点悬停显示完整路径**；头部搜索框递归搜索文件/目录名；**文件变化自动刷新**（`notify` 监听、300ms 防抖）；文件单击或右键「打开」**先探测内容是否文本**：文本在应用内预览窗口打开（语法高亮），非文本提示无法打开；右键菜单管理（打开/复制/粘贴、重命名、删除、属性、在资源管理器中打开、添加为会话附件），支持从系统剪贴板粘贴文件/文件夹，属性弹窗显示大小与时间；切换 Tab 保留展开状态。
- **Git 更改**：右侧面板底部 Tab（历史会话 / 会话资源 / **Git 更改**），后端内置 **gix**（纯 Rust，不依赖系统 Git）；以**当前会话工作目录**为根检测所在仓库，列出**已更改文件**（新增/修改/删除/未跟踪圆形状态图标，悬停提示状态文字 + 分支名；`node_modules` 无条件忽略），点击文件复用独立 diff 窗口查看（统一对比 HEAD，暂存 + 未暂存合并；未跟踪文件以空内容对比）；**分支管理**：点击分支名弹出下拉弹层，支持**切换 / 新建 / 删除本地分支**（切换由 gix 进程内完成：同步工作区文件并重建索引，有已跟踪改动或会覆盖未跟踪文件时拒绝；删除禁止删除当前分支）；**变更自动刷新**（监听仓库目录与 `.git`，300ms 防抖，`.gitignore` 规则参与事件过滤）；目录不在仓库时提供**“添加到 Git”一键初始化**（仅 `git init`，不自动提交）。
...
- **头部**：六边形 Logo 在左；右侧依次是“工作目录（显示文件夹名，悬停 tooltip 显示完整路径；仅新会话态可点击选目录，会话/历史态只读）+ 新建会话”黏连胶囊组、设置按钮（最右）；会话进行中切换会话会弹出确认。
...
│   ├── git.rs         Git 更改：状态/diff/初始化 + 仓库监听（内置 gix，零系统 git 依赖）
│   └── settings.rs    设置读写
└── Vue 3 前端  src/
    ├── composables/useCodex.ts  状态 + 事件订阅 + 协议调用
    ├── composables/useSessionFs.ts  会话资源状态（懒加载/搜索/监听）
    ├── composables/useGitChanges.ts  Git 更改状态（检测/刷新/初始化/监听）
    └── components/              界面组件（RightPanel + HistoryView + ResourceView）
```
...
- **工作目录**：从哪个目录启动 codex-ui，新建会话默认归到该目录；头部右侧显示当前目录的文件夹名，悬停可看完整路径；仅新会话态点击可改选目录，会话开始后或历史会话中为只读展示、不可点击。历史会话显示各自记录的目录，继续执行用记录目录。
