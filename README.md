...
- **Zen 代理的口嗨检测与自动续跑**：弱模型（Zen 免费模型尤其明显）常以「接下来我会…/Now update the test …」这类承诺句结束回合、却一次工具都没调用，codex 于是把回合当完成收尾，表现为 agent loop 提前退出。代理在**流式翻译路径**里补了一道终局检查：当一个上游响应以「**纯文本 + 零工具调用 + `finish_reason=stop`**」结束时（正是 codex 会就此收尾的那一刻），先用**同一个模型**发一次独立的后台判定调用（非流式、不带任何工具，提示词含本轮用户请求、助手终局文本与「本回合工具调用数为 0」的事实，要求只回一行 `未完成`/`已完成`），判为「未完成」就把提醒作为**合成 user 消息**追加到发给上游的历史里（历史尾部先补一条本轮助手文本，再补提醒「【自动续跑】…必须实际调用工具完成剩余工作…」）并**再打一次上游，把第二轮的事件续在同一个 SSE 流**上——工具调用照常下发给 codex，`response.completed` 只发一次，对 codex 与应用完全透明，注入的提醒**不会进入 codex 记录**（聊天里看不到任何伪造消息）。边界与开关：判定拿不准（超时 15 秒／报错／输出无法解析）一律按「已完成」处理，照常收尾；同一会话「连续注入且模型仍未实际调用工具」达到 **2 次**后不再注入（模型一旦真的调用工具、或距上次注入超过 10 分钟即清零），单次入站请求最多 4 个 pass 兜底；请求里没有声明任何工具时不做判定；**非流式路径、错误/失败路径、历史净化与工具翻译逻辑均不受影响**。诊断日志：`zen_proxy.nudge_judged`（verdict／回复字数／耗时／错误原因）、`zen_proxy.nudge_injected`（session／pass／streak／助手字数）、`zen_proxy.nudge_skipped`（原因：上游错误/状态码/转发失败）、`zen_proxy.nudge_limited`（max_streak／max_passes）；内容级诊断另开两份请求记录（call_id 带 `-judge<n>` / `-nudge<n>` 后缀）。未覆盖：不检测「调用过工具但没做对」的情况，也不回溯历史回合；判定期间这条流会静默最长 15 秒（不额外发 SSE 保活注释）。
