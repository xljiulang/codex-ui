...
- **启动进入设置页**：应用启动（`init()` 完成后）**无条件打开设置标签**（设置页恒在标签栏最后，可手动关闭，重新启动会再次打开）；不再展示「欢迎」介绍标签。
...
- **设置文件**：`%APPDATA%\com.codexui.app\settings.json`，仅保存 codex 路径、Enter 快捷发送、跟进处理方式、提示音开关、主题、毛玻璃特效开关、权限模式初始值、记忆模式与微信接入总开关。
