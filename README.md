...
- **配置快照（标题栏）**：在 `CODEX_HOME/codex-ui/<配置名>` 保存多个 codex 配置快照并在之间切换。**还原**：把该快照的 `config.toml` 覆盖回 `CODEX_HOME/config.toml`，并把快照中模型目录文件的内容写回 config 解析出的 `model_catalog_json` 目标文件（相对 / `~` / 绝对路径均支持），随后触发 `config/batchWrite` 热重载，toast「已应用配置快照「x」，重启 codex-ui 后生效」。**删除**：确认后删除对应快照文件夹。**打开**：把该快照目录下的 `config.toml` 与（若存在）模型目录文件在编辑器打开。**新建/覆盖（底部输入 + 新建）**：把当前 codex-home 的 `config.toml` 与 `model_catalog_json` 值内容快照到 `codex-ui/<配置名>/`，同名已存在则覆盖更新（快照内容即当前状态，**不还原文件**）；菜单保持展开刷新列表便于看到新增项。快照成员为 `config.toml`（原文复制、从不改写结构）+ 按 `model_catalog_json` 源文件名存放的模型目录文件（承载其值内容）；配置名需为合法 Windows 文件夹名。快照间不记录「当前激活」关系，因此无激活项高亮/勾选。
