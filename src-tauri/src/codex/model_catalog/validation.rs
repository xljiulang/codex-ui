...
pub const OUTPUT_FIELDS: &[&str] = &[
    "slug", "display_name", "description", "priority", "context_window",
    "max_context_window", "effective_context_window_percent", "auto_compact_token_limit",
    ...
    "include_apps_usage_instructions", "supports_parallel_tool_calls",
    "available_in_plans", "service_tiers", "additional_speed_tiers",
    "default_service_tier", "auto_review_model_override", "availability_nux", "upgrade",
];
...
pub fn reconcile_facts(facts: &mut ModelFacts, warnings: &mut Vec<String>) {
    ...
    if let (Some(context), Some(input)) = (facts.context_window, facts.input_token_limit) {
        if input > context {
            let context_quality = facts.provenance.get("context_window").map(|origin| origin.quality);
            ...
            if input_origin.as_ref().map(|origin| origin.quality) > context_quality {
                ...
                warnings.push("输入上限与上下文冲突，已按更可信的输入上限重新派生上下文".to_string());
            } else {
    ...
    if let Some(default) = &facts.default_reasoning_level {
        if !facts.reasoning_levels.as_ref().is_some_and(|levels| levels.iter()
...
pub fn validate_entry(entry: &mut Value, warnings: &mut Vec<String>) -> Result<(), String> {
    ...
    if !entry.pointer("/model_messages/instructions_template").and_then(Value::as_str)
    ...
    for key in ["experimental_supported_tools", "supported_reasoning_levels"] {
        if !entry[key].is_array() { return Err(format!("生成条目的 {key} 必须为数组")); }
    }
    if !entry["experimental_supported_tools"].as_array().unwrap().iter().all(Value::is_string) {
    ...
    for level in levels {
        let effort = level["effort"].as_str().filter(|value| !value.trim().is_empty())
    ...
    if let (Some(context), Some(maximum)) = (entry["context_window"].as_i64(), entry["max_context_window"].as_i64()) {
        if maximum < context { return Err("最大上下文小于上下文窗口".to_string()); }
    }
    ...
    let percent = entry["effective_context_window_percent"].as_i64().unwrap_or(95);
    if !(1..=100).contains(&percent) {
        return Err("有效上下文百分比必须在 1–100 之间".to_string());
    }
    if let (Some(context), Some(compact)) = (entry["context_window"].as_i64(), entry["auto_compact_token_limit"].as_i64()) {
    ...
    if let Some(modalities) = entry.get("input_modalities") {
        if !modalities.as_array().is_some_and(|values| values.iter().all(Value::is_string)) {
...
pub fn complete_provenance(entry: &Value, provenance: &mut Provenance) {
    for key in OUTPUT_FIELDS {
        ...
        let mut origin = provenance.get(*key).or_else(|| provenance.get(input)).cloned().unwrap_or_else(|| {
            let mut origin = FieldProvenance::new("template", "", MatchKind::Exact, FieldQuality::from(0));
...
mod tests {
    ...
    fn valid_entry() -> Value {
        super::super::template::render(
            ...
            "review-model", &ModelFacts::default(), 1,
        ).unwrap()
    }
    ...
    fn validates_required_shapes_context_and_compaction() {
        for (key, value) in [
            ("slug", json!("")), ("priority", json!(0)),
            ("model_messages", json!({})), ("support_verbosity", json!("false")),
            ("experimental_supported_tools", json!([1])),
            ("context_window", json!(-1)), ("max_context_window", json!(1)),
            ...
            entry[key] = value;
            assert!(validate_entry(&mut entry, &mut vec![]).is_err(), "应拒绝非法 {key}");
        }
    }
    ...
    fn conflicting_constraints_use_higher_quality_or_drop_equal_input_limit() {
        for input_authority in [20, 30] {
            let mut facts = ModelFacts {
                context_window: Some(1000), max_context_window: Some(1000),
                input_token_limit: Some(2000), ..ModelFacts::default()
            };
            facts.provenance.insert("context_window", FieldProvenance::new(
                "openrouter", "model", MatchKind::Exact, 20.into()));
            facts.provenance.insert("input_token_limit", FieldProvenance::new(
                "models_dev", "model", MatchKind::Exact, input_authority.into()));
            let mut warnings = vec![];
    ...
    fn provenance_values_describe_the_final_output_not_raw_facts() {
        let mut facts = ModelFacts {
            reasoning_levels: Some(vec!["turbo".into()]), input_modalities: Some(vec!["audio".into()]),
            ..ModelFacts::default()
        };
        facts.attach_match("models_dev", "model", MatchKind::Exact, 30.into());
        let entry = super::super::template::render(
            &super::super::template::from_snapshot(&[]).unwrap(), "model", &facts, 1).unwrap();
        complete_provenance(&entry, &mut facts.provenance);
        assert_eq!(facts.provenance["supported_reasoning_levels"].value, entry["supported_reasoning_levels"]);
        assert_eq!(facts.provenance["supported_reasoning_levels"].source, "models_dev");
        ...
        for key in ["display_name", "supported_reasoning_levels"] {
            let mut origin = FieldProvenance::new("official", "model", MatchKind::Exact, 100.into());
        ...
        assert_eq!(official["supported_reasoning_levels"].reason, "完整条目复用");
    }

    #[test]
    fn known_input_limit_is_not_lost_when_context_sources_are_missing() {
        let mut facts = ModelFacts { input_token_limit: Some(32000), ..ModelFacts::default() };
        facts.attach_match("models_dev", "model", MatchKind::Exact, 30.into());
        let mut warnings = vec![];
        reconcile_facts(&mut facts, &mut warnings);
